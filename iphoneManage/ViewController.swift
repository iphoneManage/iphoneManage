...
//        kiracmlt(sshpassPath, "-p", sshpass, "ssh", "-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null", "root@\(ipadress)", "-p", "2222", "ldrestart")
    ...
    @IBAction func installdepends(_ sender: Any) {
        ...
        kiracmlt("rm", "-rf", "~/Library/Preferences/com.kirasrcipts.iphoneManage.plist")
        kiracmlt("chmod", "+x", "/Applications/iphoneManage.app/Contents/Resources/installdepends")
        kiracmlt("/Applications/iphoneManage.app/Contents/Resources/installdepends")
...
""", "-m", """
        ...
        let answer = dialogOKCancel(question: "DONE", text: "Installing Dependencies is a bit complicated so you need to do it twice.. When you launch iManager the first time, click install dependencies, than relaunch it and tap install dependencies again")
